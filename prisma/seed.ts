...
async function main() {
  ...
  const formatEnrollment = (year: number, dep: string, branch: string) => {
          ...
          Array.from({ length: 5 }).map(async () => {
            ...
            return prisma.student.create({
              data: {
                ...
                enrollmentNumber: formatEnrollment(
                  23,
                  dept.name === 'SOT' ? '01' : dept.name === 'SOM' ? '02' : '03',
